...
def config_path():
    if hasattr(sys, '_MEIPASS'):
        ...
        return os.path.join(exe_dir, 'yuno_v5_config.json')
    return os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'yuno_v5_config.json')
...
EMOTION_DANGO = {
    ...
    'tsundere':  {'file': 'tsundere.png',  'name': '傲娇', 'scale': (1.0, 1.0),  'rot': -12,  'bounce': 0.1, 'shake': 0.0,  'stretch': 0.0, 'tilt': -1},
}

EMOTION_DURATION = {
    'calm': 0, 'happy': 3.0, 'angry': 2.5, 'sad': 3.5,
    'surprised': 2.0, 'tsundere': 4.0,
}
