...
class V7Menu(QWidget):
    ...
    def _build_ui(self):
        ...
        # V10 成长区
        layout.addWidget(self._section_title('养成状态'))
        growth_widget = QWidget()
        gl = QVBoxLayout(growth_widget)
        gl.setContentsMargins(8, 2, 8, 2)
        gl.setSpacing(4)
        self._level_label = QLabel()
        self._level_label.setStyleSheet('color: #FFD700; font-size: 9pt; font-family: "Microsoft YaHei";')
        gl.addWidget(self._level_label)
        self._exp_bar = QProgressBar()
        self._exp_bar.setFixedHeight(10)
        self._exp_bar.setTextVisible(False)
        self._exp_bar.setStyleSheet('QProgressBar { background: rgba(40,35,70,150); border-radius: 5px; } QProgressBar::chunk { background: #D4AF37; border-radius: 5px; }')
        gl.addWidget(self._exp_bar)
        self._affection_label = QLabel()
        self._affection_label.setStyleSheet('color: #FF99CC; font-size: 9pt; font-family: "Microsoft YaHei";')
        gl.addWidget(self._affection_label)
        self._affection_bar = QProgressBar()
        self._affection_bar.setFixedHeight(10)
        self._affection_bar.setTextVisible(False)
        self._affection_bar.setStyleSheet('QProgressBar { background: rgba(40,35,70,150); border-radius: 5px; } QProgressBar::chunk { background: #FF99CC; border-radius: 5px; }')
        gl.addWidget(self._affection_bar)
        food_row = QWidget()
        fr = QHBoxLayout(food_row)
        fr.setContentsMargins(0, 0, 0, 0)
        self._food_label = QLabel()
        self._food_label.setStyleSheet('color: #D0C8E0; font-size: 9pt; font-family: "Microsoft YaHei";')
        fr.addWidget(self._food_label)
        fr.addStretch()
        feed_btn = QPushButton('喂食 +10')
        feed_btn.setFixedHeight(24)
        feed_btn.setCursor(Qt.PointingHandCursor)
        feed_btn.setStyleSheet('QPushButton { background: rgba(212,175,55,200); color: #2a1f0a; border: none; border-radius: 12px; font-size: 9pt; font-weight: bold; padding: 0 12px; } QPushButton:hover { background: rgba(255,215,0,230); }')
        feed_btn.clicked.connect(lambda: (self._pet.v10_interact('feed'), self._update_growth()))
        fr.addWidget(feed_btn)
        gl.addWidget(food_row)
        layout.addWidget(growth_widget)
        layout.addSpacing(4)

        # 互动区
        layout.addWidget(self._section_title('互动区'))
        for icon_key, text, action in [('feed','喂食','feed'), ('random','玩耍','play'), ('poke','戳脸','poke')]:
            item = V7MenuItem(self._icons.get(icon_key), text)
            item.clicked.connect(lambda a=action: (self.close(), self._pet.v10_interact(a)))
