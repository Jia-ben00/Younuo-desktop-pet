...
"""
import sys, os, json, re, time, random, traceback, tempfile, subprocess, math, winsound, shutil
...
VOICE_V10_DIR = os.path.join(ASSETS, 'voice_v10')
ANIMATION_DIR = os.path.join(ASSETS, 'animations')

def growth_data_path():
    if hasattr(sys, 'frozen', False):
        return os.path.join(os.path.dirname(sys.executable), 'yuno_v10_data.json')
        ...
        if action == 'feed':
            if self.growth.can_feed():
                ...
                if success:
                    ...
                    self._v10_state = 'sajiao'
                    self.voice_v10.play_line('sajiao', tier_idx)
                    self._pet.set_bubble('吧唧吧唧~好吃！')
                    if new_level > 0:
                        self.voice_v10.play('levelup')
                        self._pet.set_bubble(f'升级啦！Lv.{new_level}')
            else:
                self._v10_state = 'shangxin'
                self.voice_v10.play_line('shangxin', tier_idx)
                self._pet.set_bubble('没有月亮糕了...')
        elif action == 'play':
            self._v10_state = 'kaixin'
            self.voice_v10.play_line('kaixin', tier_idx)
            self._pet.set_bubble('想做的就去做到~')
        elif action == 'poke':
            self._v10_state = 'daimeng'
            self.voice_v10.play_line('daimeng', tier_idx)
            self._pet.set_bubble('...何事？')
        elif action == 'drag':
            self._v10_state = 'haixiu'
            self.voice_v10.play_line('haixiu', tier_idx)
            self._pet.set_bubble('谁、谁让你碰我！')
        QTimer.singleShot(4000, lambda: self._pet.set_bubble(''))
